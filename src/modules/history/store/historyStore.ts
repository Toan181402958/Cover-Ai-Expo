...
import { useCreateVoiceStore } from "modules/home/store/createVoiceStore";
import {
  getObjectDataLocal,
  KEY_STORAGE,
  saveObjectDataLocal,
...
type HistoryStateProps = {
  ...
  resumeCreatePending: (arrayPending: Array<any>) => void;
};
export const useHistoryStore = create<HistoryStateProps>((set, get) => ({
  ...
  getDataLocal: () => {
    getObjectDataLocal(KEY_STORAGE.LOCAL_HISTORY).then((res) => {
      if (res) {
        ...
        //   }
        // });
        const listResumePending = res.filter(
          (val: any) => val.status == TYPE_STATUS_ALL.PENDING && !!val?.id
        );
        if (!!listResumePending?.length) {
          get().resumeCreatePending(listResumePending);
        }
        set({ arrHistory: res });
  ...
  clearInterval: (id) => {
    if (!!id) {
      const item = get().arrHistory.find((element) => {
        return element.id === id;
      });
      console.log("🚀 ~ arrHistory:", get().arrHistory);
      console.log("🚀 ~ item:", item);
      clearInterval(item?.intervalId);
    }
  },
  resumeCreatePending: (arrayPending) => {
    arrayPending.forEach((el: any) => {
      const data = {
        idCreate: el.idCreate,
        id: el.id,
        status: "PENDING",
        source_url: null,
        output: "",
        ...
        thumbnail_voice: el.thumbnail_voice,
        intervalId: null as NodeJS.Timeout | null,
      };
      // data.intervalId = setInterval(() => {
      useCreateVoiceStore
        .getState()
        .getStatusVoice(data.id, data.idCreate.toString());
      // }, 5000);
      // get().updateItemHistory(data);
    });
  },
}));
